<template>
  <div class="m-5 p-5 border-2 rounded-md">
    <div  class="lg:flex lg:items-center lg:justify-between ">
        <div class="flex-1 min-w-0">
            <div class="mt-2 flex items-center text-sm text-gray-500">
                <router-link to="">Qiita</router-link> 
                <ChevronRightIcon class="flex-shrink-0 mr-1.5 h-5 w-5 text-gray-400" aria-hidden="true" />
                <router-link to="">New Release</router-link>
                <ChevronRightIcon class="flex-shrink-0 mr-1.5 h-5 w-5 text-gray-400" aria-hidden="true" />
                <router-link to="">Rust</router-link> 
            </div>
            <h2 class="text-2xl font-bold leading-6 text-gray-900 sm:text-2xl sm:truncate">
                <!-- <p>{{ data.title }}</p> -->
    ...
    <div :v-if="data?.summary" class="block mt-5 w-full min-w-0">
        <h3 class="text-md text-gray-500 sm:text-md break-words" v-html="data?.summary"></h3>
    </div>
  </div>
</template>
...
import { IBachelor } from '../../utils/interface/main';
